...
object Operation {
  ...
  case class Branch(modifier: Const, length: Const, op1: Either[Const, Address], op2: Either[Const, Address], target: Const) extends Operation
  ...
  case class Set(length: Const, src: Either[Const, Address], dst: Address) extends Operation
  // `length` is treated as 8 bytes
  // todo: src can be only const or native address
  case class SetNative(length: Address, src: Either[Const, Address], dst: Address.Native) extends Operation

  // Integer arithmetic operations
  case class Add(length: Const, op1: Either[Const, Address], op2: Either[Const, Address], dst: Address) extends Operation
  case class Mult(length: Const, op1: Either[Const, Address], op2: Either[Const, Address], dst: Address) extends Operation
  case class Neg(length: Const, op: Either[Const, Address], dst: Address) extends Operation
}
