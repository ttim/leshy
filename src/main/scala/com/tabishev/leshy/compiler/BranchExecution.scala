...
import com.tabishev.leshy.node.BranchImpl
import com.tabishev.leshy.runtime.Runtime

object BranchExecution {
  final case class Const(value: Boolean) extends BranchImpl {
    override def execute(runtime: Runtime): Boolean = value
  }

  final case class MoreMM4(op1: MemoryOperand, op2: MemoryOperand) extends BranchImpl {
    override def execute(runtime: Runtime): Boolean =
      op1.materialize(runtime).getInt() > op2.materialize(runtime).getInt()
  }

  final case class MoreMC4(op1: MemoryOperand, op2: Int) extends BranchImpl {
    override def execute(runtime: Runtime): Boolean = op1.materialize(runtime).getInt() > op2
  }

  final case class MoreMM8(op1: MemoryOperand, op2: MemoryOperand) extends BranchImpl {
    override def execute(runtime: Runtime): Boolean =
      op1.materialize(runtime).getLong() > op2.materialize(runtime).getLong()
  }

  final case class MoreMC8(op1: MemoryOperand, op2: Long) extends BranchImpl {
    override def execute(runtime: Runtime): Boolean = op1.materialize(runtime).getLong() > op2
  }

  final case class LessOrEqualMM4(op1: MemoryOperand, op2: MemoryOperand) extends BranchImpl {
    override def execute(runtime: Runtime): Boolean =
      op1.materialize(runtime).getInt() <= op2.materialize(runtime).getInt()
  }

  final case class LessOrEqualMC4(op1: MemoryOperand, op2: Int) extends BranchImpl {
    override def execute(runtime: Runtime): Boolean = op1.materialize(runtime).getInt() <= op2
  }

  final case class LessOrEqualMM8(op1: MemoryOperand, op2: MemoryOperand) extends BranchImpl {
    override def execute(runtime: Runtime): Boolean =
      op1.materialize(runtime).getLong() <= op2.materialize(runtime).getLong()
  }

  final case class LessOrEqualMC8(op1: MemoryOperand, op2: Long) extends BranchImpl {
    override def execute(runtime: Runtime): Boolean = op1.materialize(runtime).getLong() <= op2
  }
