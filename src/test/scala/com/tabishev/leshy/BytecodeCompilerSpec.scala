...
class BytecodeCompilerSpec extends munit.FunSuite {
  ...
  test("test branch") {
    val intOp = Right(MemoryOperand.Stack(FrameOffset.nonNegative(0)))
    val longOp = Right(MemoryOperand.Stack(FrameOffset.nonNegative(0)))
  ...
  def opArg(value: MemoryOperand): Either[Bytes, MemoryOperand] = Right(value)
  def intArg(value: Int): Either[Bytes, MemoryOperand] = Left(Bytes.fromInt(value))
